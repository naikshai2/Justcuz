...
(function() {
	...
	function reloadTable(ds) {
		...
		var schedule = ds.schedule.map( function(n) { 
			return [n[0], '$'+n[1].toFixed(2), '$'+n[2].toFixed(2), '$'+n[3].toFixed(2),'$'+n[4].toFixed(2)];
		});
	...
	function computeSchedule(loan_amount, interest_rate, payments_per_year, years, payment) {
		...
		for (var i=1; i<=number_of_payments; i++) {
			...
			total+=interest;
			remaining -= principle;
			var row = [i, principle>0?(principle<payment?principle:payment):0, interest>0?interest:0,total>0?total:0, remaining>0?remaining:0];
			schedule.push(row);
	...
	function drawData(ds){
		var t=0;
		for (var count = 1; count < ds.numberOfPayments; count++) {
